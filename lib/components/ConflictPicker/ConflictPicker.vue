...
const props = defineProps<{
	...
	/**
	 * The incoming nodes with conflicting names compared to `content`
	 */
...
const areAllNewSelected = computed(() => newSelected.value.length === props.conflicts.length)
const areSomeNewSelected = computed(() => !areAllNewSelected.value && !isNoNewSelected.value)
const areAllOldSelected = computed(() => oldSelected.value.length === props.content.length)
const areSomeOldSelected = computed(() => !areAllOldSelected.value && !isNoNewSelected.value)
const areConflictsResolved = computed(() => {
	for (const [index, entry] of props.content.entries()) {
		if (oldSelected.value.includes(entry) || newSelected.value.includes(props.conflicts[index]!)) {
			continue
		}
		return false
	}
...
const dialogName = computed(() => props.dirname?.trim() !== ''
	? n('%n file conflict in {dirname}', '%n file conflicts in {dirname}', props.conflicts.length, { dirname: props.dirname })
	: n('%n file conflict', '%n files conflict', props.conflicts.length))
...
function onSelectAllNew(selected: boolean) {
	if (selected) {
		...
		newSelected.value = [...props.conflicts]
	} else {
...
function onSelectAllOld(selected: boolean) {
	if (selected) {
		...
		oldSelected.value = [...props.conflicts]
	} else {
	...
	<NcDialog
		...
		<form
			...
			<ConflictPickerEntry
				...
				:existing="node"
				:incoming-selected="newSelected.includes(conflicts[index]!)"
				:existing-selected="oldSelected.includes(node)"
				@update:existing-selected="toggleOldSelected(node)"
				@update:incoming-selected="toggleNewSelected(conflicts[index]!)" />
		</form>
		...
		<template #actions>
			...
			<NcButton @click="onSkipAll">
				...
				{{ conflicts.length === 1 ? t('Skip this file') : n('Skip %n file', 'Skip %n files', conflicts.length) }}
			</NcButton>
			<NcButton
				:aria-disabled="!areConflictsResolved || undefined"
				:class="[
